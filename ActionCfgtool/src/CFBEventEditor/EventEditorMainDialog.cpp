...
CEventEditorMainDialog::CEventEditorMainDialog(CWnd* pParent )
...
void CEventEditorMainDialog::SaveAllDataToEvent()
{
	...
	{
		m_pFlowControl->CFBFlowControl_GetData(&pData, nLen);
...
void CEventEditorMainDialog::OnEventHandlerSelChange(CIEventHandler* pEvent)
{
	...
	if (pEvent)
	{
		...
		{
			m_pFlowControl->CFBFlowControl_SetData(pData, nLen);
		}
...
void CEventEditorMainDialog::SetData(char* pData, int nLen)
{
	...
	int nEventCount = 0;
	CFB_MEMCPY_INC(nEventCount, pData);
	...
	for (int i=0; i<nEventCount; i++)
	{
		...
		int nEventDataLen = 0;
		// 获取长度
		CFB_MEMCPY_INC(nEventDataLen, pData);
		if (nEventDataLen)
		{
			// 数据
...
void CEventEditorMainDialog::Set3DRobotView(ICFBRobot3dView* pView)
{
...
ICFBFlowControl* CEventEditorMainDialog::GetEventFlowCtr()
{
...
void CEventEditorMainDialog::SetSocketSession(CICFBSocketSession* pSession)
{
