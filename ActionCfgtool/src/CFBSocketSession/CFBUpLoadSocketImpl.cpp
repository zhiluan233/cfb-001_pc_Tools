...
{
    CCFBUpLoadSocketImpl* pProxy = new CCFBUpLoadSocketImpl();
    if (pProxy)
    {
        *ppSession = pProxy;
        return true;
...
CCFBUpLoadSocketImpl::CCFBUpLoadSocketImpl(QObject *parent)
    : QObject(parent)
    , m_nFileLen(0)
    , m_nRecvDataLen(0)
    , m_pFile(NULL)
...
CCFBUpLoadSocketImpl::~CCFBUpLoadSocketImpl()
{
}

bool CCFBUpLoadSocketImpl::Initial(QString strIPAddr, int nPort, QString strFileName, int nFileLen, IFileTransferEvent* pEvent)
{
    m_pEvent = pEvent;
    m_nFileLen = nFileLen;
...
void CCFBUpLoadSocketImpl::displayError(QAbstractSocket::SocketError socketError)
{
    switch (socketError)
    {
...
void CCFBUpLoadSocketImpl::ReleaseConnection()
{
    m_receiveSocket->close();
    delete this;
}

void CCFBUpLoadSocketImpl::StartReadData()
{
