#include "stdafx.h"
#include "CFBBroadcastSocketImpl.h"
#include <QtNetwork>
#include <QDebug>

//#include "../LibPackClass/CFBDevInfo.h"
...
bool CreateCFBBroadcastSocket(CICFBBroadcastSocket **ppSession)
{
    CCFBBroadcastSocketImpl* pProxy = new CCFBBroadcastSocketImpl();
    if (pProxy)
    {
        *ppSession = pProxy;
        return true;
...
CCFBBroadcastSocketImpl::CCFBBroadcastSocketImpl(QObject *parent)
    : QObject(parent)
{
}

CCFBBroadcastSocketImpl::~CCFBBroadcastSocketImpl()
{
    delete udpSocket;
}

bool CCFBBroadcastSocketImpl::Initial(HWND m_hWnd, ICFBBroadcastEvent* pEvent)
{
    if (pEvent)
    {
        m_pEvent = pEvent;
...
bool CCFBBroadcastSocketImpl::Release(void)
{
//	bool bStatus =true;
//	if (m_BCRecvSock)
//	{
...
// protobuf发送消息
bool CCFBBroadcastSocketImpl::SendMsg(int nId , const std::string& msgBody)
{
...
void CCFBBroadcastSocketImpl::processPendingDatagrams()
{
...
void CCFBBroadcastSocketImpl::regDev(DevInfo &dev)
{
...
void CCFBBroadcastSocketImpl::ParseMsg(char* pMsg , int nLen)
{
    if (pMsg == NULL)
        return;
...
int CCFBBroadcastSocketImpl::ParseHeaderData(char * pData, int nLen, CHeader &header)
{
