...
#ifdef Q_OS_WIN
        strDiskName = CCFBUdiskManager::GetDiskFlag();
#else
        if (!CCFBUdiskManager::getUBTUSBDeviceByIOLib(strDiskName, strBSDPath))
        {
...
//            return;
        ...
        {
            if(CCFBUdiskManager::getUBTUSBDeviceByCommand(strDiskName, strBSDPath))
            {
