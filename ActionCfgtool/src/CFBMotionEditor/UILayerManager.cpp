...
bool CUILayerManager::ParseRawData(char* pData, int nLen)
{
    ...
    // 保存总的层数
    int nLayerCount = 0;
    CFB_MEMCPY_INC(nLayerCount, pData);
    for (int i=0; i<nLayerCount; i++)
    {
        char* pRawData = NULL;
        int nRawDataLen = 0;
        ...
        // 获取TYPE
        int nType = 0;
        CFB_MEMCPY_INC(nType, pData);

        ILayerUI* pLayer = NULL;
        AddAcionLayer(nType);
        pLayer = FindLayer(nType);
        // Raw DATA
        CFB_MEMCPY_INC(nRawDataLen, pData);
        if (nRawDataLen)
        {
...
void CUILayerManager::Set3DRobotViewInterface(ICFBRobot3DView* pRobotView)
{
...
void CUILayerManager::IUILayerManager_SetSocketSession(CICFBSocketSession* pSession)
{
...
void CUILayerManager::slotEarLedPlay(int RunTime, int LeftLed, int RightLed, int Bright, int LedLightUpTime, int LedLightDownTime , bool bEnd)
{
    if (m_pSocketSession)
    {
        m_pSocketSession->CFBSocketSession_PlayLedData(emEarLed , LeftLed , RightLed , Bright , 3 , RunTime , LedLightUpTime , LedLightDownTime, bEnd);
    }
}
...
void CUILayerManager::slotEyeLedPlay(int LeftLed, int RightLed, int Bright, int Color, int runTime, int LightUpTime, int LightDownTime, bool bEnd)
{
    if (m_pSocketSession)
    {
        m_pSocketSession->CFBSocketSession_PlayLedData(emEyeLed , LeftLed , RightLed , Bright , Color , runTime , LightUpTime , LightDownTime, bEnd);
    }
}
...
void CUILayerManager::IUILayerManager_StopPlayEyeData()
{
    if (m_pSocketSession)
    {
        m_pSocketSession->CFBSocketSession_StopPlayLedData(emEyeLed);
    }
}
...
void CUILayerManager::IUILayerManager_StopPlayEarData()
{
    if (m_pSocketSession)
    {
        m_pSocketSession->CFBSocketSession_StopPlayLedData(emEarLed);
    }
}
...
void CUILayerManager::IUILayerManager_ShowExpression(QString strFileName, int nCurFrame)
{
    if (m_pSocketSession)
    {
        m_pSocketSession->CFBSocketSession_PlayExpression(strFileName, nCurFrame);
    }
}
