...
emActionLayerType CUILayerCommonWidget::getDropDataType(const char* pData, const int nLen)
{
    ...
    int nActionTypePos = sizeof(int) * 2 + strlen(CFB_TEMPLATE_ACTION_TAG); //文件总长度，文件头长度，文件头
...
void CUILayerCommonWidget::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if(event->mimeData()->hasFormat(CFB_TEMPLATE_ACTION_TAG))
    {
        m_dragMode = 1;
...
void CUILayerCommonWidget::dragMoveEvent(QDragMoveEvent *event)
{
    if(event->mimeData()->hasFormat(CFB_TEMPLATE_ACTION_TAG))
    {
        ...
        QByteArray data = event->mimeData()->data(CFB_TEMPLATE_ACTION_TAG);
...
void CUILayerCommonWidget::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasFormat(CFB_TEMPLATE_ACTION_TAG))
    {
        ...
        int nSumLen = 0;
        CFB_MEMCPY_INC(nSumLen, pData);
        int nTagLen = 0;
        CFB_MEMCPY_INC(nTagLen, pData);
        //去Tag12
        char *pTag = new char[nTagLen + 1];
        memcpy(pTag, pData, nTagLen);
        pTag[nTagLen] = 0;
        int nRet = strncmp(pTag, CFB_TEMPLATE_ACTION_TAG, nTagLen + 1);
