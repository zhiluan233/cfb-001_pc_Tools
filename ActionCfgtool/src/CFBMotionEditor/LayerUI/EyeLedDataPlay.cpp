...
void EyeLedDataPlay::ProcessPlayActionBlock(CActionBlockData* pActionBlock, int nRunTime, int TimeScale, bool bEnd)
{
    ...
    // 左眼LED
    CFB_MEMCPY_INC(m_LeftLed, pData);
    // 右眼LED
    CFB_MEMCPY_INC(m_RightLed, pData);
    // 亮度
    CFB_MEMCPY_INC(m_LedBright, pData);
    // 颜色
    CFB_MEMCPY_INC(m_LedColor, pData);
    // 亮的时间
    CFB_MEMCPY_INC(m_LedLight, pData);
    // 灭的时间
    CFB_MEMCPY_INC(m_LedTurnOff, pData);
