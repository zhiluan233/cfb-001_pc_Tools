...
//void COptionsEarLedWidget::Set3DRobotViewInterface(ICFBRobot3DView *pRobotView)
//{
...
void COptionEyeLedWidget::SetRawData(char* pData, int nLen)
{
    ...
    // 左眼LED
    CFB_MEMCPY_INC(m_Left, pData);
    // 右眼LED
    CFB_MEMCPY_INC(m_Right, pData);
    // 亮度
    CFB_MEMCPY_INC(m_Bright, pData);
    // 眼灯颜色
    CFB_MEMCPY_INC(m_color, pData);
    // 亮的时间
    CFB_MEMCPY_INC(m_LedLigth, pData);
    // 灭的时间
    CFB_MEMCPY_INC(m_LedPutOut, pData);
}
