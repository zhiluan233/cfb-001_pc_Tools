...
CFBSlider::CFBSlider(QWidget *parent) : QSlider(parent)
{
...
void CFBSlider::enterEvent(QEvent *event)
{
    emit sigOnEnter();
    QSlider::enterEvent(event);
}

void CFBSlider::leaveEvent(QEvent *e)
{
    emit sigOnLeave();
    QSlider::leaveEvent(e);
}

void CFBSlider::mouseMoveEvent(QMouseEvent *event)
{
...
void CFBSlider::mousePressEvent(QMouseEvent *event)
{
...
inline int CFBSlider::pick(const QPoint &pt) const
{
    return orientation() == Qt::Horizontal ? pt.x() : pt.y();
}

// Function copied from qslider.cpp and modified to make it compile
void CFBSlider::initStyleOption_Qt430(QStyleOptionSlider *option) const
{
    if (!option)
        return;
...
// Function copied from qslider.cpp and modified to make it compile
int CFBSlider::pixelPosToRangeValue(int pos) const
{
    QStyleOptionSlider opt;
    initStyleOption(&opt);
