...
#include "ui_cfbrobot3dcontrolwidget.h"
...
QMap<QString, int> CFBRobot3DControlWidget::groupNameID;

CFBRobot3DControlWidget::CFBRobot3DControlWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CFBRobot3DControlWidget),
    ...
    m_bDoubleClicked(false)
{
    ...
    //        this, &CFBRobot3DControlWidget::onButtonToggled);
    ...
    connect(ui->horizontalSliderAngle, &QSlider::sliderMoved, this, &CFBRobot3DControlWidget::onSliderMoved);
...
CFBRobot3DControlWidget::~CFBRobot3DControlWidget()
{
    CUtil::ReleaseVector(m_vecDefaultMotorDataList);
    delete ui;
}

void CFBRobot3DControlWidget::UpdateNodeDate(const char *szNodeName, float angle, float offset, TypeAngleChanged nChangeType)
{
...
void CFBRobot3DControlWidget::onButtonToggled(int id, bool checked)
{
...
void CFBRobot3DControlWidget::slotServoIdClicked(int nId)
{
    if (m_bDoubleClicked)
    {
        m_bDoubleClicked = false;
...
void CFBRobot3DControlWidget::slotSingleClicked()
{
    emActionLayerType eActionType = ACTION_LAYER_UNKNOWN;
...
//void CFBRobot3DControlWidget::onSliderAnglevalueChanged(int value)
void CFBRobot3DControlWidget::onSliderMoved(int value)
{
...
void CFBRobot3DControlWidget::on_pushButtonMinus_clicked()
{
...
void CFBRobot3DControlWidget::on_pushButtonPlus_clicked()
{
    int nNewAngle = ui->horizontalSliderAngle->value()+1;
...
//void CFBRobot3DControlWidget::onSliderPressed()
//{
//    changeRobotAngle();
//}

void CFBRobot3DControlWidget::on_pushButtonReset_clicked()
{
...
void CFBRobot3DControlWidget::on_pushButtonLegReadback_clicked()
{
    if(!m_bCanRead)
        return;
    ...
    {
        m_pEvent->ICFBRobot3DViewEvent_ReadRotation(i , bPowerDown);
    }
    ...
    if(NULL == m_pTimerRead)
    {
        ...
        connect(m_pTimerRead, &QTimer::timeout, this, &CFBRobot3DControlWidget::OnTimeOutRead);
        m_pTimerRead->start(400);
    }
}


void CFBRobot3DControlWidget::on_pushButtonHandReadBack_clicked()
{
    if(!m_bCanRead)
        return;
...
void CFBRobot3DControlWidget::slotHeadReadBack()
{
    if(!m_bCanRead)
        return;
    ...
    {
        m_pEvent->ICFBRobot3DViewEvent_ReadRotation(i);
    }
...
void CFBRobot3DControlWidget::on_pushButtonReadAll_clicked()
{
    if(!m_bCanRead)
        return;
    ...
    for (item = m_vecDefaultMotorDataList.begin(); item != m_vecDefaultMotorDataList.end(); item++)
    {
        ...
        m_pEvent->ICFBRobot3DViewEvent_ReadRotation(itemData->nID);
    }
...
void CFBRobot3DControlWidget::EnableCtrls(bool bEnable)
{
#if 0
...
void CFBRobot3DControlWidget::ResetRobot()
{
...
//    SAFE_DELETE_ARRAY(pMotorData);
    ...
    for (item = m_vecDefaultMotorDataList.begin(); item != m_vecDefaultMotorDataList.end(); ++item)
    {
        ...
        emit angleChanged(tempData->nID, tempData->nAngle, 1000, TAC_RESET);
    }

    if (m_pEvent)
    {
        m_pEvent->ICFBRobot3DViewEvent_SetAnglesToRobot(pMotorData , nMotorCount , nTime , nCurveType);
    }
...
void CFBRobot3DControlWidget::ReadDefaultAngle(vector<MOTOR_DATA*>& motorlist)
{    
...
void CFBRobot3DControlWidget::readModelXml(const QDomElement& element, vector<MOTOR_DATA*>& motorlist)
{
...
void CFBRobot3DControlWidget::slotSingleReadBack()
{
    m_eReadAngleType = eReadAngleSingle;

    int nMotorID = m_btnGroup->checkedId();
    bool bPowerDown = true;
    m_pEvent->ICFBRobot3DViewEvent_ReadRotation(nMotorID , bPowerDown);
}


void CFBRobot3DControlWidget::OnPortOpenChanged(BOOL bOpen)
{
    if(bOpen)
    {
        //端口已连接
...
void CFBRobot3DControlWidget::OnTimeOutRead()
{
    if(m_pTimerRead)
    {
        delete m_pTimerRead;
...
void CFBRobot3DControlWidget::resizeEvent(QResizeEvent *event)
{
...
void CFBRobot3DControlWidget::changeRobotAngle(bool bEmitSignal, int nTime)
{
    if(m_btnGroup->checkedId() != -1 && ui->horizontalSliderAngle)
    {
        ...
        if(!m_bIsReading && m_pEvent)
        {
            ...
            m_pEvent->ICFBRobot3DViewEvent_SetRotation(nMotorID, value, nTime);
        }
        if(bEmitSignal)
        {
...
void CFBRobot3DControlWidget::timerEvent(QTimerEvent* event)
{
    //int nTimerId = event->timerId();

}

void CFBRobot3DControlWidget::ConnectToMainWidgets(ICFBRobot3DViewEvent* pEvent)
{
    m_pEvent = pEvent;
}
...
bool CFBRobot3DControlWidget::eventFilter(QObject * obj, QEvent * event)
{
